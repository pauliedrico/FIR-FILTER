...
const int bi[NT]={-1, -2, -4, 8, 35, 50, 35 , 8, -4, -2, -1}; /// b array
...
int myfilter(int x)
{
  ...
  y = 0;
  for (i=0; i<NT; i++)
    y += ((sx[i]*bi[i]) >> SHAMT) << (SHAMT-NB+1); // con approx. (parallelismo ? bit)
   // y += (sx[i]*bi[i]); // non approx. moltiplicazione (max. parallelismo: 16 bit in VHDL)
